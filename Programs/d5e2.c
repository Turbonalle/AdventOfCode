...
char **follow_instructions(char **stacks, int *instructions)
{
	...
	crates_to_move = instructions[0];
	from = instructions[1] - 1;
	to = instructions[2] - 1;
	crates = malloc((from + 2) * sizeof(char));
	if (!crates)
	{
		printf("Malloc failed!\n");
		return (0);
	...
	while (n < stack_size)
		temp++;
	crates = temp;
	stacks[from] = ft_substr(stacks[from], 0, stack_size - crates_to_move);
	stacks[to] = ft_strjoin(stacks[to], crates);
	return (stacks);
}
